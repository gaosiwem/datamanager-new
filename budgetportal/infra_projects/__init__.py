...
def import_snapshot(snapshot):
    ...
    for IRM_project_id in preprocessed_dataset:
        id = IRM_project_id["Project ID"]
        if id:
            models.InfraProject.objects.get_or_create(
                IRM_project_id= int(id), sphere_slug=snapshot.sphere.slug
            )
        if len(IRM_project_id) > 0:
...
#         id = item['Project ID']
#         if id:
#             infra_project, created = models.InfraProject.objects.get_or_create(
#                 IRM_project_id=id, sphere_slug=snapshot.sphere.slug
#             )
#             print(infra_project)

#             InfraProjectSnapshot.objects.create(
#                 irm_snapshot=snapshot,
#                 project=infra_project,
#                 project_number=item["Project No"],
#                 name=item["Project Name"],
#                 province=item["Province"],
#                 department=item["Department"],
#                 local_municipality=item["Local Municipality"],
#             )
...
#     print(f"Number of invalid rows: {len(result.invalid_rows)}")
#     print(f"Number of rows with errors: {len(result.row_errors())}")
#     return result
